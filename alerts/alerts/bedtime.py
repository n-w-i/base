...
from alerts.config import WHOOP_CORE_URL, CALENDAR_INTEL_URL
...
def _fetch_tomorrow_first_event() -> str | None:
    ...
    events = forecast[1].get("events", [])
    return events[0].get("start") if events else None


def calculate_bedtime(default_wake: str = "07:00") -> dict:
    ...
    return {
        ...
        "minutes_until_bedtime": int((bedtime - now).total_seconds() / 60),
    }
