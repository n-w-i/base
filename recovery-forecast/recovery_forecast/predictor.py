...
def _compute_baseline(recoveries: list[dict]) -> dict:
    scores = []
    hrvs = []
    rhrs = []
...
def _predict_day(current_recovery: float, day_calendar: dict | None, baseline: dict) -> dict:
    strain_delta = 0
    recovery_delta = 0
    events_affecting = []
...
def build_prediction(forecast_days: int = 3) -> dict:
    ...
    baseline = _compute_baseline(recoveries)
    ...
    for i in range(forecast_days):
        ...
        pred = _predict_day(running_recovery, cal_day, baseline)
